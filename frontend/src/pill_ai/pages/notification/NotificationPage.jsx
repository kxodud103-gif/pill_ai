...
function BottomNav({ activePath }) {
  const navigate = useNavigate();
  return (
    <nav style={{ display: "flex", alignItems: "center", justifyContent: "space-around", background: "#fff", borderTop: "1px solid #e9e7f5", padding: "8px 0 max(8px,env(safe-area-inset-bottom))", position: "fixed", bottom: 0, left: 0, right: 0, zIndex: 1000 }}>
      {NAV_MAIN.map(item => {
        const isActive = activePath === item.path;
        return (
          <button key={item.path} onClick={() => navigate(item.path)} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 3, padding: "4px 12px", background: "transparent", border: "none", cursor: "pointer", fontFamily: "var(--fn)", transition: "all .15s", flex: 1 }}>
...
export default function NotificationsPage() {
  ...
  const px = isDesktop ? 36 : isTablet ? 28 : 16;
  ...
  return (
    <>
      ...
      <div style={{ display: "flex", height: "100dvh", background: "#f0effa", fontFamily: "var(--fn)", overflow: "hidden" }}>

        {/* 사이드바: 데스크탑만 */}
        {isDesktop && <Sidebar activePath="/app/notifications" />}

        {/* 메인 콘텐츠 */}
        <main style={{
          flex: 1,
          padding: `${isDesktop ? 32 : 20}px ${px}px ${isMobile ? 100 : 32}px`,
          overflowY: "auto",
          minWidth: 0,
        }}>
            ...
            {/* ── 헤더 ── */}
          {!isDesktop ? (
            /* 모바일·태블릿: 다른 페이지들과 동일한 상단 헤더 */
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
              <div>
                <div style={{ fontSize: 12, color: "#9ca3af", marginBottom: 2, fontFamily: "var(--fn)" }}>안녕하세요 👋</div>
                <div style={{ fontSize: 18, fontWeight: 800, color: "#1a1433", fontFamily: "var(--fn)", display: "flex", alignItems: "center", gap: 8 }}>
                  알림
                  {unreadCount > 0 && (
                    <span style={{ background: "#ef4444", color: "#fff", fontSize: 11, fontWeight: 700, borderRadius: 50, padding: "2px 9px" }}>{unreadCount}</span>
                  )}
                </div>
              </div>
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                {unreadCount > 0 && (
                  <button onClick={markAllRead} style={{ padding: "7px 14px", background: "#ede9fe", border: "none", borderRadius: 10, fontSize: 12, fontWeight: 600, color: "#7c3aed", cursor: "pointer", fontFamily: "var(--fn)" }}>
                    모두 읽음
                  </button>
                )}
                {/* 날짜 칩 */}
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 24, flexWrap: "wrap", gap: 10 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
                ...
                <div>
                  <div style={{ fontSize: 24, fontWeight: 800, color: "#1a1433", display: "flex", alignItems: "center", gap: 8 }}>
                    알림
                    {unreadCount > 0 && (
                      <span style={{ background: "#ef4444", color: "#fff", fontSize: 13, fontWeight: 700, borderRadius: 50, padding: "2px 10px" }}>{unreadCount}</span>
                    )}
                  </div>
              ...
              {unreadCount > 0 && (
                <button onClick={markAllRead} style={{ padding: "8px 18px", background: "#ede9fe", border: "none", borderRadius: 10, fontSize: 13, fontWeight: 600, color: "#7c3aed", cursor: "pointer", fontFamily: "var(--fn)" }}>
                  모두 읽음
                </button>
              )}
            </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            ...
            {notifications.map(n => (
              ...
              >
                {/* 아이콘 */}
                <div style={{ width: isMobile ? 36 : 40, height: isMobile ? 36 : 40, borderRadius: 12, background: n.iconBg, display: "flex", alignItems: "center", justifyContent: "center", fontSize: isMobile ? 16 : 18, flexShrink: 0 }}>
                  {n.icon}
                </div>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 3 }}>
                    <div style={{ fontSize: isMobile ? 13 : 14, fontWeight: 700, color: "#1a1433" }}>{n.title}</div>
                    {!n.read && <div style={{ width: 7, height: 7, borderRadius: "50%", background: "#7c3aed", flexShrink: 0 }} />}
                  </div>
                  <div style={{ fontSize: isMobile ? 12 : 13, color: "#6b5f8a", lineHeight: 1.6 }}>{n.desc}</div>
                  <div style={{ fontSize: 11, color: "#9ca3af", marginTop: 5 }}>{n.time}</div>
                </div>

                {/* 삭제 버튼 */}
                <button
                  onClick={e => { e.stopPropagation(); deleteNotif(n.id); }}
                  style={{ width: 26, height: 26, background: "#f3f4f6", border: "none", borderRadius: 8, color: "#9ca3af", fontSize: 11, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, transition: "all .15s" }}
        ...
        {/* 하단 탭바 — 모바일 + 태블릿 */}
        {!isDesktop && <BottomNav activePath="/app/notifications" />}
      </div>
    </>
  );
}
...
html,body { width:100%; height:100%; font-family:var(--fn); background:#f0effa; overflow:hidden; }
