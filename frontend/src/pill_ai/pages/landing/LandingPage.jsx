    ...
    <>
      ...
      {menuOpen && (
        <div className="mp-mobile-menu">
          ...
          <button onClick={() => { goTo(1); }}>기능</button>
          <button onClick={() => { goTo(2); }}>사용법</button>
          <div className="mp-mobile-menu-divider" />
          <button className="mp-mobile-login" onClick={() => navigate("/login")}>로그인</button>
          <button className="mp-mobile-start" onClick={() => navigate("/app/home")}>무료로 시작</button>
        </div>
      )}
      ...
      >
        ...
        <section className="mp-fp-section" id="mp-sec-hero">
          <div
            ...
            style={{ transform: isMobile ? undefined : `translate(${cx * -18}px,${cy * -12}px)` }}
          >
              ...
              <TiltCard className="mp-g-card mp-card-main">
                ...
                {[
                  ...
                  <div key={i} className="mp-pill-row">
                    ...
                    {p.done === false && <span className="mp-pill-x">✕</span>}
                  </div>
                ))}
              </TiltCard>
...
/* ── 햄버거 버튼 (모바일) ── */
.mp-hamburger{display:none;flex-direction:column;gap:5px;background:none;border:none;cursor:pointer;padding:8px;z-index:210;}
.mp-hamburger span{display:block;width:22px;height:2px;background:var(--mp-text);border-radius:2px;transition:all .25s;}
.mp-hamburger span.open:nth-child(1){transform:translateY(7px) rotate(45deg);}
.mp-hamburger span.open:nth-child(2){opacity:0;}
.mp-hamburger span.open:nth-child(3){transform:translateY(-7px) rotate(-45deg);}

/* ── 모바일 드롭다운 메뉴 ── */
.mp-mobile-menu{display:none;position:fixed;top:64px;left:0;right:0;z-index:199;background:rgba(240,238,255,0.97);backdrop-filter:blur(20px);border-bottom:1px solid var(--mp-border);flex-direction:column;padding:16px 24px 20px;gap:4px;animation:mpFadeDown .2s ease both;}
@keyframes mpFadeDown{from{opacity:0;transform:translateY(-8px)}to{opacity:1;transform:translateY(0)}}
.mp-mobile-menu button{background:none;border:none;font-family:var(--mp-font);font-size:15px;font-weight:600;color:var(--mp-sub);padding:12px 8px;text-align:left;cursor:pointer;border-radius:10px;transition:all .2s;}
.mp-mobile-menu button:hover{background:var(--mp-pill-l);color:var(--mp-pill);}
.mp-mobile-menu-divider{height:1px;background:var(--mp-border);margin:8px 0;}
.mp-mobile-login{color:var(--mp-pill)!important;border:1.5px solid rgba(124,58,237,0.28)!important;text-align:center!important;}
.mp-mobile-start{background:linear-gradient(135deg,var(--mp-a1),#818cf8)!important;color:#fff!important;text-align:center!important;box-shadow:0 4px 14px rgba(167,139,250,0.42);}
...
/* ── CTA ── */
.mp-sec-cta{flex-direction:column;gap:0;align-items:center;justify-content:center;}
.mp-cta-inner{width:clamp(480px,34.9vw,670px);height:clamp(440px,57.1vh,490px);background:rgba(255,255,255,0.92);border:0.89px solid rgba(167,139,250,0.18);border-radius:30px;box-shadow:0 16px 48px rgba(120,80,200,0.14);position:relative;overflow:hidden;transition:transform .35s,box-shadow .35s;}
...
.mp-cta-title{position:absolute;top:8.8%;left:50%;transform:translateX(-50%);width:90%;text-align:center;font-size:clamp(28px,2.9vw,42px);font-weight:800;letter-spacing:0.1em;color:#1a1433;white-space:nowrap;font-family:var(--mp-font);}
.mp-cta-qr-img{margin-top:-30px; position:absolute;top:50%;left:50%;transform:translate(-50%,-58%);width:clamp(140px,12.5vw,240px);height:clamp(140px,12.5vw,240px);display:flex;align-items:center;justify-content:center;}
.mp-cta-qr-img svg{width:100%;height:100%;}
.mp-cta-qr-label{margin-top:-20px;position:absolute;top:59%;left:50%;transform:translateX(-50%);font-size:clamp(14px,1.25vw,24px);font-weight:700;color:#1a1433;white-space:nowrap;text-align:center;font-family:var(--mp-font);}
.mp-cta-qr-desc1{margin-top:-15px;position:absolute;top:66.6%;left:50%;transform:translateX(-50%);font-size:clamp(12px,1.04vw,20px);font-weight:400;color:#6b5f8a;white-space:nowrap;text-align:center;font-family:var(--mp-font);}
...
.mp-btn-cta{width:clamp(160px,14vw,269px);height:clamp(55px,3.5vh,54px);border-radius:50px;background:linear-gradient(135deg,rgba(167,139,250,1),rgba(129,140,248,1));border:none;color:#fff;font-size:clamp(13px,1.25vw,24px);font-weight:700;cursor:none;box-shadow:0 6px 24px rgba(167,139,250,0.45);transition:all .25s;font-family:var(--mp-font);display:flex;align-items:center;justify-content:center;}
...
@media (max-width: 1024px) {
  ...
  .mp-fp-footer{padding:0 32px;}
  .mp-footer-link-wrap{right:32px;}

  #mp-sec-hero{padding:64px 32px 0;gap:24px;}
  .mp-hero-left{flex:0 0 auto;padding-right:0;}
  .mp-hero-title{font-size:40px;}
  .mp-hero-desc{white-space:normal;font-size:14px;}

  .mp-cards-scene{width:360px;height:360px;}
  .mp-card-main{width:240px;}
  .mp-card-warn{width:160px;}
  .mp-card-search{width:170px;}

  .mp-sec-features{padding:0 32px;}
  .mp-feat-grid{grid-template-columns:repeat(2,1fr);}
  .mp-sec-title{font-size:36px;}

  .mp-sec-how{gap:48px;padding:0 32px;}
  .mp-how-left{flex:0 0 360px;}
  .mp-sec-title[style]{font-size:34px!important;}

  .mp-cta-inner{width:clamp(320px,80vw,540px);height:clamp(380px,70vh,550px);}
  .mp-fp-dots{right:12px;}
}


/* ════════════════════════════════════════════════
   반응형 — 태블릿 가로 (769px ~ 900px): hero 좌우, how 중앙
════════════════════════════════════════════════ */
@media (min-width: 769px) and (max-width: 900px) {
  ...
  .mp-hero-title{font-size:32px;white-space:normal;}
  .mp-hero-desc{white-space:normal;font-size:13px;}
  .mp-hero-right{
    flex:0 0 48%;
    width:48%;
    margin-top:0;
  ...
  .mp-cards-scene{width:100%;height:300px;max-width:300px;overflow:visible;}
  .mp-card-main{width:190px;top:50%;left:50%;transform:translate(-50%,-50%);}
  .mp-card-warn{width:138px;top:4px;right:0;left:auto;bottom:auto;transform:rotate(3deg);}
  .mp-card-search{width:142px;bottom:4px;left:0;top:auto;right:auto;transform:rotate(-2deg);}

  /* how it works: 세로 스택 + 정중앙 */
  .mp-sec-how{
  ...
  .mp-how-left{flex:0 0 auto;width:100%;max-width:560px;}
  .mp-sec-title[style]{font-size:30px!important;text-align:center!important;}
  .mp-sec-sub[style]{text-align:center!important;margin:0 0 20px!important;}
  .mp-how-right{width:100%;max-width:560px;}
  .mp-stats-box{padding:24px 28px;}
}
  .mp-pill-row{
  display:flex;
...
@media (max-width: 768px) {
  ...
  /* nav */
  .mp-nav{padding:0 20px;height:58px;}
  .mp-nav-links,.mp-nav-actions{display:none;}
  .mp-hamburger{display:flex;}
  .mp-mobile-menu{display:flex;}
  .mp-fp-dots{display:none;}
  ...
  .mp-hero-badge{margin-bottom:12px;font-size:12px;}
  .mp-hero-title{font-size:28px;white-space:normal;margin-bottom:10px;letter-spacing:0.02em;margin-top:50px;}
  .mp-hero-desc{white-space:normal;font-size:13px;margin-bottom:18px;line-height:1.65;}
  .mp-hero-br{display:none;}
  .mp-hero-cta{justify-content:center;flex-wrap:wrap;gap:8px;}
  .mp-btn-hero,.mp-btn-ghost-hero{cursor:pointer;font-size:13px;padding:10px 20px;}
  ...
  .mp-card-main{
    ...
    animation:mpFloatM 5s ease-in-out infinite;
  }
  /* 경고 카드: 오른쪽, 위치를 씬 안에 고정 */
  .mp-card-warn{
    width:190px;
    top:10px;
    right:-160px;
    ...
    animation:mpFloatW 6s 0.8s ease-in-out infinite;
  }
  /* 검색 카드: 왼쪽 아래, 씬 안에 고정 */
  .mp-card-search{
    width:195px;
    ...
    right:auto;
    transform:rotate(-2deg);
    animation:mpFloatS 5.5s 1.5s ease-in-out infinite;
  }
  .mp-scroll-hint{bottom:10px;font-size:11px;}

  /* features */
  .mp-sec-features{padding:0 20px;}
  .mp-feat-grid{grid-template-columns:1fr 1fr;gap:12px;}
  .mp-feat-card{padding:20px 16px;}
  .mp-feat-card h3{font-size:14px;}
  .mp-feat-card p{font-size:12px;}
  .mp-feat-icon{width:42px;height:42px;margin-bottom:12px;}
  .mp-sec-title{font-size:26px;letter-spacing:0.04em;}
  .mp-sec-sub{font-size:13px;margin-bottom:24px;}
  ...
  .mp-how-left{flex:0 0 auto;width:100%;max-width:520px;margin-top:100px;}
  .mp-sec-title[style]{font-size:24px!important;letter-spacing:0.03em!important;text-align:center!important;}
  .mp-sec-sub[style]{text-align:center!important;margin:0 0 14px!important;font-size:13px!important;}
  .mp-steps-v{gap:12px;align-items:flex-start;max-width: 360px;width: 100%; margin-top:50px; }
  .mp-step-v{align-items:center;}
  .mp-step-v-num{width:36px;height:36px;font-size:14px;flex-shrink:0;}
  .mp-step-v-text h4{font-size:14px;}
  .mp-step-v-text p{font-size:12px;line-height:1.5;}
  .mp-how-right{width:100%;max-width:520px;}
  .mp-stats-box{padding:16px 16px 12px;margin-top:50px;}
  .mp-stat-row{margin-bottom:16px;}
  .mp-stat-n{font-size:20px;}
  .mp-stat-l{font-size:11px;}
  .mp-chart-bars{height:70px;}
  .mp-chart-label-row{margin-bottom:8px;font-size:12px;}

  /* cta */
  .mp-sec-cta{padding:0 20px;justify-content:center;align-items:center;}
  .mp-cta-inner{width:min(425px,90vw);height:450px;margin:0 auto;}
    .mp-cta-qr-img svg{
    margin-top:-60px;}
  .mp-cta-title{font-size:clamp(18px,5vw,28px);}
  .mp-cta-btn-row{flex-direction:column;gap:10px;top:70%;}
  .mp-btn-cta,.mp-btn-cta-g{cursor:pointer;width:clamp(180px,60vw,200px);height:45px;}
  ...
  .mp-fp-footer p{position:static;transform:none;font-size:11px;white-space:normal;text-align:center;}
  .mp-footer-link-wrap{position:static;gap:12px;}
  .mp-footer-link{font-size:12px;}
  .mp-cta-qr-label{top:50%;}
  .mp-cta-qr-desc1{top:50%;margin-top:10px;}
  .mp-cta-qr-desc2{top:60%;margin-top:-5px;}
}
/* ════════════════════════════════════════════════
   반응형 — 소형 모바일 (≤ 375px)
════════════════════════════════════════════════ */
...
.wl-label{
margin-top:100px;}
.mp-pdot{margin-top:5px;}
  .mp-hero-title{font-size:26px;}
  .mp-hero-desc{font-size:13px;}
  .mp-feat-grid{grid-template-columns:1fr;}
  .mp-cards-scene{height:180px;}
  .mp-card-main{width:232px; z-index:100; margin-top:107px;}
  .mp-card-warn{width:156px;left:180px; margin-top:228px; z-index:101;}
  .mp-card-search{width:183px;left:-20px; opacity:0.8 margin-top:35px; z-index:101;}
  .mp-sec-title{font-size:22px;}
  .mp-cta-inner{height:clamp(320px,80vh,395px); width:300px;}
}
`;
